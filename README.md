...
The code in the `/main_project/` folder was used for the final results. First, the python files `Confounding factors with coordinates.py` and `ccn_fraction_confounded.py` need to be executed in order to the neural networks that are trained on different amounts of augmented data. The models can then be used by `Grad_cam.py` and `results_analysis.py` to gather the results.  
